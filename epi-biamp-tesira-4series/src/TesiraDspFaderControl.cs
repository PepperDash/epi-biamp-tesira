        ...
        IVolumeComponent
    {
        ...
        public override bool IsSubscribed
        {
            get
            {
                ...
                {
                    Debug.LogVerbose(this, "{0} is {1} and {2}", item.Key, item.Value.Enabled ? "Enabled" : "Disabled", item.Value.Subscribed ? "Subscribed" : "Not Subscribed");
                    if (item.Value.Enabled && !item.Value.Subscribed)
                    {
                        trackingBool = false;
                    }
                }
                Debug.LogDebug(this, "Item is {0}.", trackingBool ? "subscribed" : "not subscribed");
                return trackingBool;
        ...
        private void Initialize(TesiraFaderControlBlockConfig config)
        {
            ...
            Debug.LogVerbose(this, "Adding LevelControl '{0}'", Key);
            ...
            {
                ControlType = 0;
                Debug.LogVerbose(this, "{0} has BOTH Mute and Level", Key);
            }
            else if (!HasMute && HasLevel)
            {
                ControlType = 1;
                Debug.LogVerbose(this, "{0} has Level ONLY", Key);

            }

            else if (HasMute && !HasLevel)
            {
                Debug.LogVerbose(this, "{0} has MUTE ONLY", Key);
                ControlType = 2;
            }
        ...
        public override void Unsubscribe()
        {
            try
            {
                ...
                if (HasMute)
                {
                    Debug.LogDebug(this, "Unsubscribe from Mute");
            ...
            {
                Debug.LogMessage(0, this, "Exception : {0}", e.Message);
            }
        }
        ...
        {
            Debug.LogDebug(this, "Parsing Data - Name: {0} - Value {1}", customName, value);
        ...
        public override void ParseGetMessage(string attributeCode, string message)
        {
            try
            {
                Debug.LogVerbose(this, "Parsing Message - '{0}' : Message has an attributeCode of {1}", message, attributeCode);
                // Parse an "+OK" message
                ...
                Debug.LogDebug(this, "Response: '{0}' Value: '{1}'", attributeCode, value);
                ...
                switch (attributeCode)
                {
                    case "minLevel":
                    {
                        ...
                        Debug.LogDebug(this, "MinLevel is '{0}'", MinLevel);
                    ...
                    case "maxLevel":
                    {
                        ...
                        Debug.LogDebug(this, "MaxLevel is '{0}'", MaxLevel);
                    ...
                    case "level":
                    {
                        ...
                        Debug.LogDebug(this, "VolumeLevel is '{0}'", VolumeLevel);

                        break;
                    }
                    default:
                    {
                        Debug.LogMessage(0, "Response does not match expected attribute codes: '{0}'", message);
            ...
            {
                Debug.LogVerbose("Unable to parse message: '{0}'\n{1}", message, e);
            }
        ...
        {
            Debug.LogDebug(this, "volume: {0}", level);
        ...
        {
            if (!HasLevel) return;
            Debug.LogVerbose("VolumeDown Sent for {0}", LevelControlPointTag);
            if (press)
            {
                if (_volDownPressTracker)
                {
        ...
        {
            if (!HasLevel) return;
            Debug.LogVerbose("VolumeUp Sent for {0}", LevelControlPointTag);
        ...
        public override void LinkToApi(BasicTriList trilist, uint joinStart, string joinMapKey, EiscApiAdvanced bridge)
        {
            ...
            Debug.LogVerbose("TesiraChannel {0} connect", Key);
            
            if (!Enabled) return;

            var genericChannel = this as IBasicVolumeWithFeedback;

            Debug.LogVerbose(this, "TesiraChannel {0} Is Enabled", Key);
